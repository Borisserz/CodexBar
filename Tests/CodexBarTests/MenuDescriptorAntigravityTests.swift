...
struct MenuDescriptorAntigravityTests {
    ...
    @Test
    func `antigravity auto selected account explains identity free agy skip`() throws {
        let suite = "MenuDescriptorAntigravityTests-identity-free-skip"
        let defaults = try #require(UserDefaults(suiteName: suite))
        defaults.removePersistentDomain(forName: suite)
        ...
        settings.addTokenAccount(provider: .antigravity, label: "Work", token: "token")
        ...
        #expect(lines.contains(AntigravityAutoGuidance.selectedAccountLimitsUnavailable))
    }

    @Test
    func `antigravity auto injected credentials explain identity free agy skip`() throws {
        let suite = "MenuDescriptorAntigravityTests-injected-identity-free-skip"
        let defaults = try #require(UserDefaults(suiteName: suite))
        defaults.removePersistentDomain(forName: suite)
